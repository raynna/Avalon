	...
	public void processNPC() {
		...
		if (isCantInteract()) {
			if (startCountdown > 0) {
				startCountdown--;
				if (startCountdown == 0) {
					//TODO YkLagorThunderousCombat.sendMagicalAttack(this, true);
			...
			if (distractedMages >= 4 && Utils.random(5) == 0) {
				animate(new Animation(14443));
				gfx(new Graphics(166));
			...
			animate(new Animation(14442));
			gfx(new Graphics(165));
			if (Utils.random(33) == 0 && !getPossibleTargets().isEmpty()) {
				...
				mage.playSound(QUOTES_SOUNDS[index], 2);
			}
	...
	public void playMusic() {
		for (Player player : getManager().getParty().getTeam()) {
			if (player.isDead() || !getManager().isAtBossRoom(player))
				continue;
			player.getMusicsManager().forcePlayMusic(909); //fixed
		}
	}
