...
public final class HillGiant extends MobRewardNodeBuilder {

	public HillGiant() {
		super(new Object[] { "Hill giant", -1 });
	}
	...
	public void populate(Player player) {
		...
		case -1:
		case COMMON:
			createNodeBatch(1, node(IRON_ARROW, 3), 
					node(IRON_DAGGER, 1), 
					...
					nodeB(ONION_SEED, 4, 8), 
					node(TOMATO_SEED, 3), 
					node(STRAWBERRY_SEED, 2), 
					node(CABBAGE_SEED, 4), 
					nodeB(COINS, 8, 15, 38, 52, 88, 310), 
					node(COINS, 350, 493), 
					node(LIMPWURT_ROOT, 1));
			break;
		case VERY_RARE:
			createNodeBatch(1, node(STEEL_PLATEBODY, 1), 
					node(ADAMANT_ARROW, 1, 5), 
		...
		case RARE:
			createNodeBatch(1, node(IRON_2H_SWORD, 1), 
					...
					node(WATERMELON_SEED, 1, 4), 
					node(SWEETCORN_SEED, 3), 
					node(LONG_BONE, 1));
			break;
		case UNCOMMON:
			createNodeBatch(1, node(STEEL_ARROW, 10, 15), 
					node(IRON_KITESHIELD, 1), 
