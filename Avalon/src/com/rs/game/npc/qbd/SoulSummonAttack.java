...
public final class SoulSummonAttack implements QueenAttack {
	...
	public int attack(QueenBlackDragon npc, Player victim) {
		...
		npc.temporaryAttribute().put("_last_soul_summon", npc.getTicks() + Utils.random(41, 100));
		int count = npc.getPhase() - 1;
		if (count == 3) {
			count = 4;
		}
		if (npc.getSouls().size() < count) {
			victim.getPackets().sendGameMessage((count - npc.getSouls().size()) < 2
			...
			for (int i = npc.getSouls().size(); i < count; i++) {
				npc.getSouls().add(new TorturedSoul(npc, victim,
	...
	public boolean canAttack(QueenBlackDragon npc, Player victim) {
		Integer last = (Integer) npc.temporaryAttribute().get("_last_soul_summon");
