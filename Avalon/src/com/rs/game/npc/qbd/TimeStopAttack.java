...
public final class TimeStopAttack implements QueenAttack {
	...
	public int attack(final QueenBlackDragon npc, final Player victim) {
		...
		soul.gfx(TorturedSoul.TELEPORT_GRAPHIC);
		soul.animate(TorturedSoul.TELEPORT_ANIMATION);
		soul.setLocked(true);
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (stage == 8) {
					stop();
					npc.temporaryAttribute().put("_time_stop_atk", npc.getTicks() + Utils.random(50) + 40);
		...
		npc.temporaryAttribute().put("_time_stop_atk", 9999999);
	...
	public boolean canAttack(QueenBlackDragon npc, Player victim) {
		...
		Integer tick = (Integer) npc.temporaryAttribute().get("_time_stop_atk");
