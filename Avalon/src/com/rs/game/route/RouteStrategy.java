...
public abstract class RouteStrategy {
	 ...
	 */
	protected static boolean checkWallDecorationInteract(int[][] clip, int currentX, int currentY, int sizeXY,
			int targetX, int targetY, int targetType, int targetRotation) {
	 ...
	 */
	protected static boolean checkWallInteract(int[][] clips, int currentX, int currentY, int sizeXY, int targetX,
			int targetY, int targetType, int targetRotation) {
		// TODO refactor
		if (sizeXY == 1) {
			if (currentX == targetX && currentY == targetY)
			...
			if (targetType == 0) {
				if (targetRotation == 0) {
					...
					if (targetX >= currentX && targetX <= i_66_ && targetY + 1 == currentY)
						return true;
					if (currentX == targetX - sizeXY && targetY >= currentY && targetY <= i_67_
	 ...
	 */
	protected static boolean checkFilledRectangularInteract(int[][] clip, int currentX, int currentY, int sizeX,
			int sizeY, int targetX, int targetY, int targetSizeX, int targetSizeY, int accessBlockFlag) {
		// TODO refactor
