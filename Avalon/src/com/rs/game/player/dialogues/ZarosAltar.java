...
public class ZarosAltar extends Dialogue {
	...
	public void start() {
		if (!player.getPrayer().isAncientCurses())
			sendOptionsDialogue("Change from prayers to curses?", "Yes, replace my prayers with curses.",
					"Never mind.");
		else
			sendOptionsDialogue("Change from curses to prayers?", "Yes, replace my curses with prayers.",
					"Never mind.");
	}
	...
	public void run(int interfaceId, int componentId) {
		if (componentId == OPTION_1) {
			if (!player.getPrayer().isAncientCurses()) {
				...
				sendDialogue("The altar eases its grip on your mid. The curses slip from",
