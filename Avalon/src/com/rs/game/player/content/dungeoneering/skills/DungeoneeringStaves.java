...
import com.rs.game.player.dialogues.dungeoneering.DungRunecraftingD;
...
public class DungeoneeringStaves extends Action {
	...
	public boolean start(Player player) {
		...
		if (player.getSkills().getLevel(Skills.RUNECRAFTING) < levelReq) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need a Runecrafting level of " + levelReq + " in order to imbue this stave.");
			return false;
		}
	...
	public int processWithDelay(Player player) {
		...
		player.getInventory().addItem(new Item(DungRunecraftingD.RUNES[3][index]));
