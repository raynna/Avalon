	...
	*/
	...
	public boolean processObjectClick1(Player player, WorldObject object) {
		if (object.getDefinitions().name.equals("Switch")) {
			...
			if (isComplete()) {
				player.getPackets().sendGameMessage("The lever doesn't seem to respond.");
				return false;
			}
			if (resetTask == null)
				addResetTask();
			...
			if (leverTicks == maxTicks) {
				...
				if (leverCount != 5) {
					...
					for (Player player : manager.getParty().getTeam()) {
						player.getPackets().sendGameMessage("You hear a loud noise and all the switches toggle back off.");
						if (player.withinDistance(manager.getTile(reference, 7, 8), 2) || !manager.getCurrentRoomReference(new WorldTile(player)).equals(reference))
							continue;
						World.sendGraphics(player, new Graphics(2759), new WorldTile(player));
						player.animate(new Animation(13694));
