...
public class WeaponPoison {
	...
	private static final int[][] POISON = new int[][] { { 187, 5937, 5940 }, { 17572, 17596, 17620 } };
	...
	private static boolean poison(Player player, Item item, int type, boolean dungeoneering) {
		Weapon weapon = getWeapon(item.getId());
		if (weapon == null)
			return false;
		int amt = item.getAmount() > 1 * 15 ? 1 * 15 : item.getAmount();
		...
		player.getInventory().addItem(dungeoneering ? 17490 : 229, 1);
		player.getDialogueManager().startDialogue("SimpleItemMessage", weapon.poisonIds[type], "You dip the tip of the " + item.getName() + " in the weapon poison.");
		return true;
	}
