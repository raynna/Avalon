...
public class GemCutting extends Action {
    ...
    public enum Gem {
        ...
        private static Gem[] gems = Gem.values();
        ...
        public static Gem getGemById(int uncutId) {
            for (Gem gem : gems) {
                if (gem == null)
                    continue;
                if (gem.getUncut() == uncutId)
                    return gem;
            }
            return null;
        }
    ...
    public static Gem getUncut(Item first, Item second) {
        Gem gem = Gem.getGemById(first.getId());
        if (gem == null)
            gem = Gem.getGemById(second.getId());
        return gem;
    }
    ...
    public boolean checkAll(Player player) {
        ...
        if (!player.getInventory().containsOneItem(gem.getUncut())) {
            player.getPackets().sendGameMessage("You don't have any " + ItemDefinitions.getItemDefinitions(gem.getUncut()).getName().toLowerCase() + " to cut.");
            return false;
        }
        return true;
    }
