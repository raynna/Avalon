...
public class BankShop implements Serializable {
	...
	private static final long MAX_BANK_SIZE = Settings.FREE_TO_PLAY ? 68 : 468;
	...
	public boolean removeItem(int[] slot, int quantity, boolean refresh, boolean forceDestroy) {
		...
		boolean destroyed = false;
		if (item.getDefinitions().isNoted()) {
			player.getPackets().sendGameMessage("Placeholder for item " + item.getName() + " deleted.");
			if (bankTabs[slot[0]].length == 1 && (forceDestroy || bankTabs.length != 1)) {
				destroyTab(slot[0]);
				if (refresh)
					refreshTabs();
			...
			if (quantity >= item.getAmount() && item.getDefinitions().canBeNoted()) {
				player.getPackets().sendGameMessage("Placeholder for item " + item.getName() + " created.");
				int certId = ItemDefinitions.getItemDefinitions(item.getId()).getCertId();
				bankTabs[slot[0]][slot[1]] = new Item(certId, 1);
				if (refresh)
					refreshItems();
				return false;
			}
	...
	public byte[] getActualPin() {
		return actualPin;
	}

	public void setActualPin(byte[] actualPin) {
		this.actualPin = actualPin;
	}

	public byte[] getRequestedPin() {
		return requestedPin;
	}

	public void setRequestedPin(byte[] requestedPin) {
