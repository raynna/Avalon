...
public class InventoryOptionsHandler {
	...
	public static void handleItemOption3(Player player, int slotId, int itemId, Item item) {
		...
		if (itemId >= 18349 && itemId <= 18363) {
			player.getCharges().checkPercentage("Your " + ItemDefinitions.getItemDefinitions(itemId).getName() + " has "
					+ ChargesManager.REPLACE + "% charges left.", itemId, false);
		} else if (itemId == 9013) {
	...
	public static void handleItemOption7(Player player, int slotId, int itemId, Item item) {
		...
		if (!name.contains(" 0")) {
			if (name.contains("verac") || name.contains("dharok") || name.contains("ahrim") || name.contains("karil")
					|| name.contains("guthan") || name.contains("torag")) {
		...
		player.setRouteEvent(new RouteEvent(npc, new Runnable() {
			...
			public void run() {
				...
				if (npc.getId() == 11460 && item.getId() == 617) {
					if (!player.isMember()) {
						player.getDialogueManager().startDialogue("SimpleItemDialogue", 617, 1,
								"Richard likes what you offered him. In return he has given a gift of 1 Month of Avalon Membership!");
						return;
					} else {
				...
				if (npc.getId() == 9711) {
					...
					for (ItemStore data : ItemStore.values()) {
						if (data == null)
							continue;
						if (data.getDegradedItem() != null && data.getDegradedItem().getId() == item.getId()
								&& data.getHits() == 30000) {
						...
						if (data.getCurrentItem().getId() == item.getId() && data.getHits() == 30000) {
							...
							if (player.getCharges().getCharges(item.getId()) == data.getHits()) {
								player.getDialogueManager().startDialogue("RewardsTrader", npc.getId(), item.getId(),
										3);
								return;
							}
							player.getDialogueManager().startDialogue("RewardsTrader", npc.getId(), item.getId(), 2);
