...
public class RsBitsBuffer extends RsByteBuffer {
    ...
    static int[] BIT_MASK = { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535, 131071, 262143, 524287, 1048575, 2097151, 4194303, 8388607, 16777215, 33554431, 67108863, 134217727, 268435455, 536870911, 1073741823, 2147483647, -1 };
	...
	try {
	    int i_1_ = ((buffer[index * 385051775] - this.aClass449_9781.method5916((byte) 1)) & 0xff);
	    if (i_1_ < 128)
		return false;
	    return true;
	}
	...
	try {
		...
		is[i + i_4_] = (byte) ((buffer[(index += 116413311) * 385051775 - 1]) - this.aClass449_9781.method5914(223815758));
	}
	catch (RuntimeException runtimeexception) {
	...
	try {
		...
		i_8_ += ((buffer[i_6_++] & BIT_MASK[i_7_]) << numBits - i_7_);
		numBits -= i_7_;
	    }
	    if (numBits == i_7_)
		i_8_ += buffer[i_6_] & BIT_MASK[i_7_];
	    else
		i_8_ += buffer[i_6_] >> i_7_ - numBits & BIT_MASK[numBits];
	    return i_8_;
	}
	catch (RuntimeException runtimeexception) {
	...
	try {
	    index = (this.anInt9780 * 1560311085 + 7) / 8 * 116413311;
	}
	catch (RuntimeException runtimeexception) {
	...
	try {
	    int i_9_ = ((buffer[(index += 116413311) * 385051775 - 1] - this.aClass449_9781.method5914(223815758)) & 0xff);
	    if (i_9_ < 128)
		return i_9_;
	    return ((i_9_ - 128 << 8) + (((buffer[(index += 116413311) * 385051775 - 1]) - this.aClass449_9781.method5914(223815758)) & 0xff));
	}
	catch (RuntimeException runtimeexception) {
	...
	try {
	    this.anInt9780 = index * -1472569640;
	}
	catch (RuntimeException runtimeexception) {
	...
	try {
	    buffer[(index += 116413311) * 385051775 - 1] = (byte) (i + this.aClass449_9781.method5914(223815758));
	}
	catch (RuntimeException runtimeexception) {
